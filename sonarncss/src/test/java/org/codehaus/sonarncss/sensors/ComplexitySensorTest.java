...
public class ComplexitySensorTest {
  ...
  public void testNoBranches() {
    ...
    assertEquals(3, res.getMeasures().getComplexity());
  }

  @Test
  public void testSimpleBranches() {
    Resource res = SonarNcss.analyze(getFile("/metrics/branches/SimpleBranches.java"));
    assertEquals(15, res.getMeasures().getComplexity());
    assertEquals(2.14, res.getMeasures().getAvgMethodCmp(), 0.01);

    Resource simpleSwitch = res.find("simpleSwitch()", JavaType.METHOD);
    assertEquals(3, simpleSwitch.getMeasures().getComplexity());
  }
  ...
  public void testInstanceAndStaticInitBlocks() {
    ...
    assertEquals(2, res.getMeasures().getComplexity());
  }
}
