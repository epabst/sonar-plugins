...
public class CutoffFilterTest {
  ...
  public void shouldUsePeriodIfDateIsNotSet() {
    Configuration conf = new PropertiesConfiguration();
    conf.setProperty(CutoffConstants.PERIOD_IN_HOURS_PROPERTY, "10");
    assertThat(new CutoffFilter(conf).getCutoffDate().getTime(), greaterThan(System.currentTimeMillis() - 11 * 60 * 60 * 1000));
    assertThat(new CutoffFilter(conf).getCutoffDate().getTime(), lessThan(System.currentTimeMillis() - 9 * 60 * 60 * 1000));
  }
